...
if (y['pipelines']['ID-ALU'] == False and y['pipelines']['ALU-M1'] == False):
    s = f'''
...
class Wrapper(Elaboratable):
    def __init__(self):
        ...
        self.inst_mem_addr = Signal({y['pc_bit_width']},reset=0x0)
        self.inst_mem_rdata = Signal({y['value_bit_width']}, reset =0x0)
        self.wmask = Signal(4, reset=0x0)
        self.csb_alu = Signal(1, reset =0x0)
        self.csb_mem = Signal(1, reset =0x0)  
        self.web = Signal(1, reset =0x0)
        self.data_mem_wdata = Signal({y['value_bit_width']}, reset =0x0)
        self.data_mem_addr = Signal({y['data_mem_bit_width']}, reset =0x0)
        self.data_mem_rdata = Signal({y['value_bit_width']}, reset = 0x0)
        self.alu_result = Signal({y['value_bit_width']}, reset = 0x0)
        self.gpio_pins = Signal({y['value_bit_width']}, reset = 0x0)
        self.output_pins = Signal({y['value_bit_width']}, reset = 0x0)
        ...
        with m.Else(): 
            ...
            with m.If(self.pc < 7):
                ...
                with m.If(self.pc < 5):
                    ...
                    m.d.comb += self.reg_file.write_alu.eq(Const(0))
                ...
                with m.Else():
                    m.d.comb += self.reg_file.csb_alu.eq(self.ALU.csb)
                    m.d.comb += self.reg_file.write_alu.eq(self.ALU.load_wb)
            ...
            # m.d.comb += self.IF_ID_pipeline.inst_prev.eq(self.ID.instruction)

            m.d.comb += self.ID.instruction.eq(self.IF_ID_pipeline.inst_out)

            m.d.comb += self.stall_unit.csb0.eq(self.M1_M2_pipeline.csb_out)
            m.d.comb += self.stall_unit.web0.eq(self.M1_M2_pipeline.web_out)
            m.d.comb += self.stall_unit.ID_src1.eq(self.ID.s1)
            m.d.comb += self.stall_unit.ID_src2.eq(self.ID.s2)
            m.d.comb += self.stall_unit.next_dest0.eq(self.M1_M2_pipeline.reg_file_write_addr_in)
            ...
            m.d.comb += self.forwarding_alu.des0.eq(self.reg_file.write_addr_mem)
            m.d.comb += self.forwarding_alu.val0.eq(self.reg_file.write_data_mem)
            m.d.comb += self.forwarding_alu.csb0.eq(self.reg_file.csb_mem)
            m.d.comb += self.forwarding_alu.web0.eq(self.reg_file.web_mem)
            ...
            m.d.comb += self.csb_alu.eq(self.ALU.csb)
            m.d.comb += self.csb_mem.eq(self.M1_M2_pipeline.csb_out)
            m.d.comb += self.web.eq(self.ALU.web)
            m.d.comb += self.data_mem_addr.eq(self.ALU.result)
            m.d.comb += self.wmask.eq(self.ALU.wmask)
            m.d.comb += self.alu_result.eq(self.ALU.result)
            m.d.comb += self.data_mem_wdata.eq(self.ALU.Rb)

            m.d.comb += self.M1_M2_pipeline.reg_file_write_in.eq(self.ALU.load_wb)
            m.d.comb += self.M1_M2_pipeline.memory_reg_addr_out_out.eq(self.ALU.reg_addr_out)
            m.d.comb += self.M1_M2_pipeline.s1_in.eq(self.ALU.s1)
            m.d.comb += self.M1_M2_pipeline.s2_in.eq(self.ALU.s2)
            m.d.comb += self.M1_M2_pipeline.csb_in.eq(self.ALU.csb)
            m.d.comb += self.M1_M2_pipeline.web_in.eq(self.ALU.web)
            ...
            m.d.comb += self.reg_file.write_addr_alu.eq(self.ALU.reg_addr_out)
            m.d.comb += self.reg_file.write_data_alu.eq(self.ALU.result)
            ...
            m.d.comb += self.gpio_pins.eq(self.reg_file.reg[30])
...
if __name__ == "__main__":
    ...
    for i in range(2**{y['pc_bit_width']}):

        if(i<len(inst)):
            instruction = "0x"+inst[i]
            instructions.append(Signal({y['value_bit_width']},reset=int(instruction,16)))

        else:
            instructions.append(Signal({y['value_bit_width']},reset=0))

    InstMemory = Array(instructions) #IM + DM
    DataMemory = Memory(width = {y['value_bit_width']}, depth = 1024, init = [0 for i in range(1024)])

    Inst_mem_rdata = Signal({y['value_bit_width']}) #input to the wrapper
    Data_mem_rdata = Signal({y['value_bit_width']})
    data_mem_wdata = Signal({y['value_bit_width']})

    inst_mem_addr = Signal({y['pc_bit_width']})
    data_mem_addr = Signal({y['data_mem_bit_width']})
    alu_result = Signal({y['value_bit_width']})
    csb = Signal(1)
    web = Signal(1)
    wmask = Signal(4)
    ...
    m.d.comb += wrapper.read_flag.eq(Const(1))
    ...
    #comment in these lines for simulations
...
elif (y['pipelines']['ID-ALU'] == False and y['pipelines']['ALU-M1'] == True):
    s=f'''
...
elif (y['pipelines']['ID-ALU'] == True and y['pipelines']['ALU-M1'] == True):
    s=f'''
...
from all_stages.reg_file import Register_file
from all_stages.ALU_M1_pipeline import ALU_M1_Pipeline
from all_stages.ID_ALU_pipeline import ID_ALU_Pipeline
from all_stages.IF_ID_pipeline import IF_ID_Pipeline
from all_stages.M1_M2_pipeline import M1_M2_Pipeline
from all_stages.M2_WB_pipeline import M2_WB_Pipeline
from all_stages.forward_alu import Forwarding_ALU
...
class Wrapper(Elaboratable):
    def __init__(self):
        self.pc = Signal({y['pc_bit_width']}, reset=0x0)
        self.read_flag = Signal(1, reset=0x0)
        self.inst_mem_addr = Signal({y['pc_bit_width']},reset=0x0)
        self.inst_mem_rdata = Signal({y['value_bit_width']}, reset =0x0)
        self.wmask = Signal(4, reset=0x0)
        ...
        self.M1_M2_pipeline = M1_M2_Pipeline()
        self.M2_WB_pipeline = M2_WB_Pipeline()
        self.ID_ALU_pipeline = ID_ALU_Pipeline()
        self.IF_ID_pipeline = IF_ID_Pipeline()
        self.ALU_M1_pipeline = ALU_M1_Pipeline()     
    ...
    def elaborate(self,platform:Platform) -> Module:
        ...
        m.submodules.stall_unit = self.stall_unit
        m.submodules.IF_ID_pipeline = self.IF_ID_pipeline
        m.submodules.ID_ALU_pipeline = self.ID_ALU_pipeline
        m.submodules.M1_M2_pipeline = self.M1_M2_pipeline
        m.submodules.ALU_M1_pipeline = self.ALU_M1_pipeline
        m.submodules.M2_WB_pipeline = self.M2_WB_pipeline
        ...
        with m.Else(): 
            ...
            with m.If(self.pc < 7):
                ...
                m.d.comb += self.reg_file.write_addr_mem.eq(0b00000)
                m.d.comb += self.reg_file.write_data_mem.eq(0x00000000)
                m.d.comb += self.reg_file.csb_mem.eq(Const(0))
                m.d.comb += self.reg_file.web_mem.eq(Const(0))
                m.d.comb += self.reg_file.write_mem.eq(Const(0))
                m.d.comb += self.csb_mem.eq(0b1)
            ...
            with m.If(self.pc < 6):
                m.d.comb += self.forwarding_alu.des0.eq(0b00000)
                m.d.comb += self.forwarding_alu.val0.eq(0x00000000)
                m.d.comb += self.forwarding_alu.csb0.eq(0b1)
                m.d.comb += self.forwarding_alu.web0.eq(0b0)
                m.d.comb += self.forwarding_mem.des0.eq(0b00000)
                m.d.comb += self.forwarding_mem.val0.eq(0x00000000)
                m.d.comb += self.forwarding_mem.csb0.eq(0b1)
                m.d.comb += self.forwarding_mem.web0.eq(0b0)
                ...
                m.d.comb += self.stall_unit.csb0.eq(0b1)
                m.d.comb += self.stall_unit.web0.eq(0b0)
                m.d.comb += self.stall_unit.next_dest0.eq(0b00000)
                m.d.comb += self.stall_unit.csb1.eq(0b1)
                m.d.comb += self.stall_unit.web1.eq(0b0)
                m.d.comb += self.stall_unit.next_dest1.eq(0b00000)

                m.d.comb += self.M2_WB_pipeline.reg_file_write_in.eq(0b0)
                m.d.comb += self.M2_WB_pipeline.memory_reg_addr_out_out.eq(0b00000)
                m.d.comb += self.M2_WB_pipeline.s1_in.eq(0b00000)
                m.d.comb += self.M2_WB_pipeline.s2_in.eq(0b00000)
                m.d.comb += self.M2_WB_pipeline.csb_in.eq(0b1)
                m.d.comb += self.M2_WB_pipeline.web_in.eq(0b0)

                m.d.comb += self.reg_file.csb_alu.eq(Const(0))
                m.d.comb += self.reg_file.write_alu.eq(Const(0))  #pos->comb

                m.d.comb += self.csb_alu.eq(0b1)
                m.d.comb += self.web.eq(0b0)
                m.d.comb += self.wmask.eq(0b0000)
                m.d.comb += self.data_mem_addr.eq(0b0000000000)
                m.d.comb += self.alu_result.eq(0x00000000)

                m.d.comb += self.M1_M2_pipeline.reg_file_write_in.eq(0b0)
                m.d.comb += self.M1_M2_pipeline.memory_reg_addr_out_out.eq(0b00000)
                m.d.comb += self.M1_M2_pipeline.s1_in.eq(0b00000)
                m.d.comb += self.M1_M2_pipeline.s2_in.eq(0b00000)
                m.d.comb += self.M1_M2_pipeline.csb_in.eq(0b1)
                m.d.comb += self.M1_M2_pipeline.web_in.eq(0b0)

                m.d.comb += self.reg_file.write_addr_alu.eq(0b00000)
                m.d.comb += self.reg_file.write_data_alu.eq(0x00000000)

                m.d.comb += self.IF_ID_pipeline.branch.eq(Const(0))
                m.d.comb += self.IF_ID_pipeline.jump.eq(Const(0))
                m.d.comb += self.IF_ID_pipeline.stall.eq(Const(0))
                m.d.comb += self.id_mux.stall.eq(Const(0))
                m.d.comb += self.pc_controller.ra.eq(0x00000000)
                m.d.comb += self.pc_controller.immediate.eq(0x00000000)
                m.d.comb += self.id_mux.branch.eq(0b0)
                m.d.comb += self.id_mux.jump.eq(0b0)
                m.d.comb += self.forwarding_alu.des1.eq(0b00000)
                m.d.comb += self.forwarding_alu.val1.eq(0x00000000)
                m.d.comb += self.forwarding_alu.csb1.eq(0b1)
                m.d.comb += self.forwarding_alu.web1.eq(0b0)
                m.d.comb += self.ALU.Ra.eq(0x00000000)
                m.d.comb += self.ALU.Rb.eq(0x00000000)
                m.d.comb += self.ALU.reg_addr_in.eq(0b00000)
                m.d.comb += self.ALU.immediate.eq(0x00000000)
                m.d.comb += self.ALU.inst_type.eq(0b000)
                m.d.comb += self.ALU.inst_type0.eq(0b00000000000000000)
                m.d.comb += self.ALU.inst_type1.eq(0b00000000000)
                m.d.comb += self.ALU.inst_type2.eq(0b00000000000)
                m.d.comb += self.ALU.inst_type3.eq(0b0000000)
                m.d.comb += self.ALU.s1.eq(0b00000)
                m.d.comb += self.ALU.s2.eq(0b00000)
                m.d.comb += self.ALU.shamt.eq(0b00000)
                m.d.comb += self.forwarding_mem.src.eq(0b00000)
                m.d.comb += self.forwarding_mem.src_data.eq(0x00000000)

            with m.Else():
                m.d.comb += self.forwarding_alu.des0.eq(self.M1_M2_pipeline.reg_file_write_addr_in)
                m.d.comb += self.forwarding_alu.val0.eq(self.data_mem_rdata)
                m.d.comb += self.forwarding_alu.csb0.eq(self.M1_M2_pipeline.csb_out)
                m.d.comb += self.forwarding_alu.web0.eq(self.M1_M2_pipeline.web_out)
                m.d.comb += self.forwarding_mem.des0.eq(self.M1_M2_pipeline.reg_file_write_addr_in)
                m.d.comb += self.forwarding_mem.val0.eq(self.data_mem_rdata)
                m.d.comb += self.forwarding_mem.csb0.eq(self.M1_M2_pipeline.csb_out)
                m.d.comb += self.forwarding_mem.web0.eq(self.M1_M2_pipeline.web_out)
                ...
                m.d.comb += self.stall_unit.csb0.eq(self.ALU.csb)
                m.d.comb += self.stall_unit.web0.eq(self.ALU.web)
                m.d.comb += self.stall_unit.next_dest0.eq(self.ALU.reg_addr_out)
                m.d.comb += self.stall_unit.csb1.eq(self.ALU_M1_pipeline.memory_csb)
                m.d.comb += self.stall_unit.web1.eq(self.ALU_M1_pipeline.memory_web)
                m.d.comb += self.stall_unit.next_dest1.eq(self.ALU_M1_pipeline.memory_reg_addr_out_in)
                ...
                m.d.comb += self.pc_controller.ra.eq(self.ALU.Ra)
                m.d.comb += self.pc_controller.immediate.eq(self.ALU.immediate)
                m.d.comb += self.id_mux.branch.eq(self.ALU.branching)
                m.d.comb += self.id_mux.jump.eq(self.ALU.jump)
                m.d.comb += self.forwarding_alu.des1.eq(self.ALU.reg_addr_out)
                m.d.comb += self.forwarding_alu.val1.eq(self.ALU.result)
                m.d.comb += self.forwarding_alu.csb1.eq(self.ALU.csb)
                m.d.comb += self.forwarding_alu.web1.eq(self.ALU.web)
                m.d.comb += self.ALU.Ra.eq(self.ID_ALU_pipeline.RA_out)
                m.d.comb += self.ALU.Rb.eq(self.ID_ALU_pipeline.RB_out)
                m.d.comb += self.ALU.reg_addr_in.eq(self.ID_ALU_pipeline.ID_ALU_reg_addr_out)
                m.d.comb += self.ALU.immediate.eq(self.ID_ALU_pipeline.immediate_out)
                m.d.comb += self.ALU.inst_type.eq(self.ID_ALU_pipeline.instruction_type_out)
                m.d.comb += self.ALU.inst_type0.eq(self.ID_ALU_pipeline.it0_out)
                m.d.comb += self.ALU.inst_type1.eq(self.ID_ALU_pipeline.it1_out)
                m.d.comb += self.ALU.inst_type2.eq(self.ID_ALU_pipeline.it2_out)
                m.d.comb += self.ALU.inst_type3.eq(self.ID_ALU_pipeline.it3_out)
                m.d.comb += self.ALU.s1.eq(self.ID_ALU_pipeline.s1_out)
                m.d.comb += self.ALU.s2.eq(self.ID_ALU_pipeline.s2_out)
                m.d.comb += self.ALU.shamt.eq(self.ID_ALU_pipeline.shamt_out)
                m.d.comb += self.forwarding_mem.src.eq(self.ALU.s2)
                m.d.comb += self.forwarding_mem.src_data.eq(self.ALU.Rb)
            ...
            m.d.comb += self.ID_ALU_pipeline.RA_in.eq(self.forwarding_alu.ra)
            m.d.comb += self.ID_ALU_pipeline.RB_in.eq(self.forwarding_alu.rb)
            m.d.comb += self.ID_ALU_pipeline.ID_ALU_reg_addr_in.eq(self.id_mux.des)
            m.d.comb += self.ID_ALU_pipeline.immediate_in.eq(self.id_mux.signextended_immediate)
            m.d.comb += self.ID_ALU_pipeline.instruction_type_in.eq(self.id_mux.instruction_type)
            m.d.comb += self.ID_ALU_pipeline.it0_in.eq(self.id_mux.it0)
            m.d.comb += self.ID_ALU_pipeline.it1_in.eq(self.id_mux.it1)
            m.d.comb += self.ID_ALU_pipeline.it2_in.eq(self.id_mux.it2)
            m.d.comb += self.ID_ALU_pipeline.it3_in.eq(self.id_mux.it3)
            m.d.comb += self.ID_ALU_pipeline.s1_in.eq(self.id_mux.s1)
            m.d.comb += self.ID_ALU_pipeline.s2_in.eq(self.id_mux.s2)
            m.d.comb += self.ID_ALU_pipeline.shamt_in.eq(self.id_mux.shamt)
...
if __name__ == "__main__":
    ...
    Data_mem_rdata = Signal({y['value_bit_width']})
    data_mem_wdata = Signal({y['value_bit_width']})
    # mem_instr = Signal(1) #output from the wrapper
